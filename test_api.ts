...
console.log("56 Marriages length:", personData._family.marriage.length);
if (personData._family.marriage.length > 0) {
	console.log("56 bio children of first marriage:", personData._family.marriage[0].children.bio.map(c => c.id));
}

const person28 = buildPersonObject('28', allData);
console.log("28 Full siblings:", person28._family.siblings.full.map(c => c.id));
